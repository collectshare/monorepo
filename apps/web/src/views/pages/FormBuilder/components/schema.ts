...
const textFieldSchema = z.object({
  ...
  text: z.string({ message: 'O nome da pergunta é obrigatório' }).min(1, 'O nome da pergunta é obrigatório'),
...
const optionFieldSchema = z.object({
  ...
  options: z.array(
    z.string({ message: 'As opções devem ser preenchidas' })
      .trim(),
  )
...
export const formBuilderFormData = z.object({
  name: z.string({ message: 'O nome é obrigatório' }).min(1, 'O nome é obrigatório'),
