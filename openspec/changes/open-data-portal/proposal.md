...
- Novo app `apps/portal` (Vite + React + TS, público, sem auth): página Home com busca semântica via Algolia (search-only key) e página de Dataset com tabela paginada dos dados brutos.
...
- **`apps/api`**:
  ...
  - Novo `infra/gateways/AlgoliaGateway.ts`, novas env vars em `shared/config/env.ts` / `AppConfig.ts`.
  - Novos `application/controllers/portal/{GetPublishedFormController,GetPublishedFormDataController}.ts`, `application/queries/GetPublishedFormDataQuery.ts`, `main/functions/portal/*.ts`, `sls/functions/portal.yml`.
...
- **Novo `apps/portal`**: app completo (rotas, services, `httpClient`, `algoliaClient`).
- **Novo `packages/ui`**: pacote de componentes compartilhados.
- **Infra**: nova tabela de stream trigger no `serverless.yml`/`sls/functions/form.yml`, novas variáveis de ambiente (Algolia), novo conjunto de rotas HTTP públicas.
- **Dependências novas**: `algoliasearch` (SDK) em `apps/api` e em `apps/portal`.
